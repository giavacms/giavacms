(function () {
    ...
    var module = angular.module("giavacms-date");

    module.controller('MyDatePickerController', function ($mdDialog, $mdMedia, $scope, currentDate) {
            var self = this;
    ...
    module.factory("MyDatePicker", ["$mdDialog", function ($mdDialog) {
        var datePicker = function (targetEvent, currentDate) {
            ...
            return $mdDialog.show({
                ...
                '<md-button aria-label="mese precedente" class="md-icon-button" ng-click="datepicker.prevMonth()">        <md-icon md-font-icon="icon-chevron-left"></md-icon>     </md-button>' +
                '{{ datepicker.currentMoment.format("MMMM YYYY") }}' +
                '<md-button aria-label="mese successivo" class="md-icon-button" ng-click="datepicker.nextMonth()">        <md-icon md-font-icon="icon-chevron-right"></md-icon>    </md-button>' +
                '</div>' +
    ...
    module.directive("myDatePicker", function ($timeout, MyDatePicker) {
        return {
            restrict: 'A',
            require: '?ngModel',
            link: function (scope, element, attrs, ngModel) {
                if ('undefined' !== typeof attrs.type && 'date' === attrs.type && ngModel) {
                    angular.element(element).on("click", function (ev) {
                        ev.preventDefault();
                        MyDatePicker(ev, ngModel.$modelValue).then(function (selectedDate) {
                            $timeout(function () {
