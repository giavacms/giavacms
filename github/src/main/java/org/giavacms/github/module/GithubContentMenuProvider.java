...
public class GithubContentMenuProvider implements MenuProvider {
	...
	@Inject
	GithubContentRepository richContentRepository;
	...
	public List<MenuValue> getMenuItemValues(String name) {
		if (name.compareTo("richcontent") == 0) {
			List<GithubContent> list = richContentRepository.getAllList();
			...
			Search<GithubContent> search = new Search<GithubContent>(
					new GithubContent());
			List<GithubContent> list = richContentRepository.getList(search, 0,
					0);
			List<MenuValue> menuList = new ArrayList<MenuValue>();
			if (list != null && list.size() > 0)
				for (GithubContent richContent : list) {
