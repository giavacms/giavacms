...
public class BaseProducer implements Serializable
{
   ...
   {
      if (items.get(ResourceType.class) == null || items.get(ResourceType.class).length == 0)
      {
         List<SelectItem> valori = new ArrayList<SelectItem>();
         for (ResourceType t : ResourceType.values())
         {
            if (t.getFolder() != null && !t.getFolder().isEmpty())
            {
               valori.add(new SelectItem(t.getFolder(), t.getDescription()));
            }
         }
         items.put(ResourceType.class, valori.toArray(new SelectItem[] {}));
      }
      return items.get(ResourceType.class);
