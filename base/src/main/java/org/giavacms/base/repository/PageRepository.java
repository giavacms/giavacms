...
@Stateless
@LocalBean
public class PageRepository extends AbstractPageRepository<Page> {

	// VAI SU!!!
	private static final long serialVersionUID = 1L;

	protected void applyRestrictionsNative(Search<Page> search,
			String pageAlias, String templateImplAlias, String templateAlias,
		...
		if (search.getObj().getTemplate() != null
				&& search.getObj().getTemplate().getTemplate() != null
				&& search.getObj().getTemplate().getTemplate().getId() != null) {
			sb.append(separator).append(templateAlias)
					.append(".id = :idTemplate ");
			params.put("idTemplate", search.getObj().getTemplate()
					.getTemplate().getId());
			separator = " and ";
		}
		...
		if (search.getObj().getId() == null
				|| search.getObj().getId().trim().isEmpty()) {
			...
			if (!search.getObj().isExtended()) {
				...
				// separator = " and ";
				sb.append(separator).append(" ( ").append(pageAlias)
						.append(".extension = :extension ) ");
				params.put("extension", Page.EXTENSION);
				separator = " and ";

				// NO CLONI DI PAGINE BASE (DOVREBBE ESSERE SUPERFLUO)
				sb.append(separator).append(pageAlias)
						.append(".clone = :clone ");
		...
		super.applyRestrictionsNative(search, pageAlias, separator, sb, params,
				customLike);
	...
	public List<Page> getExtensions(String extension) {
		try {
			StringBuffer sbq = new StringBuffer(
					"select p.id, p.title, p.template_id from "
							+ Page.TABLE_NAME
							+ " p where p.active = :ACTIVE and p.clone = :CLONE and p.extension ");
			if (extension != null && extension.trim().length() > 0) {
				sbq.append(" = :EXTENSION ");
			} else {
				sbq.append(" is null ");
			}
			Query q = getEm().createNativeQuery(sbq.toString())
					.setParameter("ACTIVE", true).setParameter("CLONE", false);
			if (extension != null && extension.trim().length() > 0) {
	...
	public String getBasePageTitleByTemplateImplId(Long templateImplId) {
		try {
			return (String) getEm()
					...
					.setParameter("TID", templateImplId)
					.setParameter("CLONE", false).getSingleResult();
		} catch (Exception e) {
			return null;
		}
	}
	...
	protected Page extract(Object[] row, boolean completeFetch) {
		...
		page.setLang(page.getId().equals(page.getLang1id()) ? 1 : page.getId()
				...
				page.getLang3id()) ? 3
				: page.getId().equals(page.getLang4id()) ? 4 : page.getId()
						.equals(page.getLang5id()) ? 5 : 0);
	 ...
	 * distinct results
	 */
	protected StringBuffer getListNative(Search<Page> search,
		...
		sb.append(" FROM ").append(Page.TABLE_NAME).append(" ")
				.append(pageAlias);
		sb.append(" LEFT JOIN ");
		sb.append(TemplateImpl.TABLE_NAME).append(" ")
				.append(templateImplAlias).append(" on ").append(pageAlias)
				.append(".template_id = ").append(templateImplAlias)
				.append(".id ");
		sb.append(" LEFT JOIN ");
		sb.append(Template.TABLE_NAME).append(" ").append(templateAlias)
				.append(" on ").append(templateImplAlias)
				.append(".template_id = ").append(templateAlias).append(".id ");

		String separator = " where ";
		applyRestrictionsNative(search, pageAlias, templateImplAlias,
				templateAlias, separator, sb, params);
	...
	public boolean resetLanguage(int l, String id) {
		try {
			int affectedRows = getEm()
					.createNativeQuery(
							"update " + Page.TABLE_NAME + " set lang" + l
									+ "id = null where id = :ID")
	...
	public boolean updateLanguageByTemplate(int langId, Long templateImplId,
			boolean set) {
		try {
			if (langId > 0) {
				int affectedRows = getEm()
						.createNativeQuery(
								"update " + Page.TABLE_NAME + " set lang"
										+ langId + "id = "
										+ (set ? " id " : " NULL ")
		...
		try {
			if (langId > 0) {
				getEm().createNativeQuery(
						"update " + Page.TABLE_NAME + " set lang" + langId
								+ "id = :alternatePageId where id = :PID ")
						.setParameter("PID", pageId)
						.setParameter("alternatePageId", alternatePageId)
						.executeUpdate();
			}
		} catch (Exception e) {
			logger.error(e.getMessage());
