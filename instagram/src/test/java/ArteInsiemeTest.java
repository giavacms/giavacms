...
public class ArteInsiemeTest {
	...
	public static void main(String[] args) throws Exception {
		...
		TagCountResult tagResult = SearchMediaCountByTag.execute(TOKEN,
				"arteinsieme", true);
		scrivi(fileName, tagResult.getData().getName() + " - num. :"
				+ tagResult.getData().getMediaCount() + A_CAPO);
		int i = 0;
		int numFoto = 0;
		...
		while (ancora) {
			...
			for (Media media : result.getData()) {
				...
				scrivi(fileName, "----------------foto n." + numFoto
						+ "----------------" + A_CAPO);
				scrivi(fileName, "username:" + media.getUser().getUsername()
						+ A_CAPO);
				scrivi(fileName, "fullName:" + media.getUser().getFullName()
						+ A_CAPO);
				scrivi(fileName, "bio:" + media.getUser().getBio() + A_CAPO);
				scrivi(fileName, "----------------" + media.getId()
						+ "----------------" + A_CAPO);

				scrivi(fileName, "foto: "
				...
				String imgExt = media
						...
						.getUrl()
						.substring(
								media.getImages().getStandardResolution()
										.getUrl().lastIndexOf(".") + 1);
				writeImage(
						"/home/fiorenzo/jworks/workspace_jboss/giava-cms/instagram/docs/img/",
						media.getUser().getUsername() + "_"
								+ users.get(media.getUser().getUsername()),
						imgExt, media.getImages().getStandardResolution()
								.getUrl());

				scrivi(fileName, "Thumb: "
						+ media.getImages().getThumbnail().getUrl() + A_CAPO);
				writeImage(
						"/home/fiorenzo/jworks/workspace_jboss/giava-cms/instagram/docs/thumb/",
						media.getUser().getUsername() + "_"
								+ users.get(media.getUser().getUsername()),
						imgExt, media.getImages().getStandardResolution()
								.getUrl());
				scrivi(fileName, "**********************" + A_CAPO);
			}
