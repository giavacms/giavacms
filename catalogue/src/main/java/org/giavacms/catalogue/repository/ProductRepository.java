...
public class ProductRepository extends AbstractPageRepository<Product> {
	...
	protected StringBuffer getListNative(Search<Product> search,
		...
		sb.append(" LEFT JOIN ").append(Category.TABLE_NAME).append(" AS ")
				...
				.append(".category_id ) ");
		sb.append(" LEFT JOIN ").append(Page.TABLE_NAME).append(" as ")
				.append(pageAlias).append(" on ( ").append(productAlias)
				.append(".id = ").append(pageAlias).append(".id ) ");
		sb.append(" LEFT JOIN ").append(Page.TABLE_NAME).append(" as ")
				.append(categoryPageAlias).append(" on ( ")
				.append(categoryAlias).append(".id = ")
				.append(categoryPageAlias).append(".id ) ");
		sb.append(" LEFT JOIN ").append(TemplateImpl.TABLE_NAME).append(" as ")
				.append(templateImplAlias).append(" on ( ")
				.append(templateImplAlias).append(".id = ").append(pageAlias)
				.append(".template_id ) ");
		...
		if (count) {
			...
			sb.append(" LEFT JOIN ").append(Page.TABLE_NAME).append(" as ")
					.append(innerPageAlias).append(" on ( ")
					.append(innerProductAlias).append(".id = ")
					.append(innerPageAlias).append(".id ) ");
			sb.append(" LEFT JOIN ").append(Page.TABLE_NAME).append(" as ")
					.append(innerCategoryPageAlias).append(" on ( ")
					.append(innerCategoryAlias).append(".id = ")
					.append(innerCategoryPageAlias).append(".id ) ");
			sb.append(" LEFT JOIN ").append(TemplateImpl.TABLE_NAME)
					.append(" as ").append(innerTemplateImplAlias)
					.append(" on ( ").append(innerTemplateImplAlias)
					.append(".id = ").append(innerPageAlias)
					.append(".template_id ) ");
		} else {
	...
	protected void applyRestrictionsNative(Search<Product> search,
		...
		if (search.getObj().getTitle() != null
			...
			params.put("LIKETEXTCUSTOM", likeParam(search.getObj().getTitle()
					.trim().toUpperCase()));
		}
	 ...
	 * sb.append(productAlias).append(".dimensions, ");
	 * sb.append(productAlias).append(".price, ");
	 * sb.append(productAlias).append
	 * (".vat, ");sb.append(productAlias).append(".code, ");
	 * sb.append(productAlias).append(".category_id, ");
	 * sb.append(categoryPageAlias).append(".title AS categoryTitle, ");
	 * sb.append(imageAlias).append(".id AS imageId, ");
	 * sb.append(imageAlias).append(".fileName AS image");
		...
		while (results.hasNext()) {
			...
			if (clone != null) {
				if (clone instanceof Boolean) {
					...
					product.setClone(((Short) clone).intValue() > 0 ? true
							: false);
				} else {
					logger.error("clone instance of "
			...
			if (template_impl_id != null) {
				if (template_impl_id instanceof BigInteger) {
					product.getTemplate().setId(
							((BigInteger) template_impl_id).longValue());
					product.setTemplateId(((BigInteger) template_impl_id)
							.longValue());
				} else {
					logger.error("templateImpl_id instance of "
							+ template_impl_id.getClass().getCanonicalName());
				}
			} else {
			...
			if (imageId != null) {
				if (imageId instanceof BigInteger) {
					...
					logger.error("imageId instance of "
							+ imageId.getClass().getCanonicalName());
				}
			}
			i++;
			...
			if (documentId != null) {
				if (documentId instanceof BigInteger) {
					...
					logger.error("documentId instance of "
							+ documentId.getClass().getCanonicalName());
				}
			}
			i++;
			String documentfileName = (String) row[i];
			if (document != null && documentfileName != null
					&& !documentfileName.isEmpty()) {
	...
	public boolean destroy(Product t) {
		try {
			getEm().createNativeQuery(
					"delete from " + Product.TABLE_NAME + "_"
							+ Image.TABLE_NAME + " where "
							+ Product.class.getSimpleName() + "_id = :ID ")
					.setParameter("ID", t.getId()).executeUpdate();
			getEm().createNativeQuery(
					"delete from " + Product.TABLE_NAME + "_"
							+ Document.TABLE_NAME + " where "
							+ Product.class.getSimpleName() + "_id = :ID ")
					.setParameter("ID", t.getId()).executeUpdate();
			getEm().createNativeQuery(
					"delete from " + Product.TABLE_NAME + " where id = :ID ")
					.setParameter("ID", t.getId()).executeUpdate();
			getEm().createNativeQuery(
					"delete from " + Page.TABLE_NAME + " where id = :ID ")
					.setParameter("ID", t.getId()).executeUpdate();
			return true;
		} catch (Exception e) {
