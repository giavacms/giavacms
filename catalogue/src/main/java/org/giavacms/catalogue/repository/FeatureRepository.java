...
public class FeatureRepository extends AbstractRepository<Feature>
{
            ...
            String separator, StringBuffer sb, Map<String, Object> params)
   {
      ...
      if (search.getObj().getName() != null && search.getObj().getName().trim().length() > 0)
      {
      ...
      if (search.getObj().getOption() != null && search.getObj().getOption().trim().length() > 0)
      {
         sb.append(separator).append(" upper ( ").append(alias).append(".option ) like :OPTION ");
         params.put("OPTION", likeParam(search.getObj().getOption().trim().toUpperCase()));
         separator = " and ";
      }
   ...
   public List<String> getRefereanceableNames()
   {
      try
      {
         return getEm()
                  .createQuery(
                           "select distinct p.name from " + Feature.class.getSimpleName()
                                    + " p where p.active = :ACTIVE order by p.name asc ").setParameter("ACTIVE", true)
                  .getResultList();
      }
      catch (Exception e)
      {
